...
logic_error STRUCTLostENDM ("Unexpected STRUCT without ENDM");      // *JiK*
logic_error STRUCTnested ("Nested STRUCT is not supported yet");    // *JiK**
//**************************************************************

// Errors in the code being assembled.

runtime_error InvalidBANK ("Invalid BANK value or out of bounds");  // *JiK*
runtime_error RedefinedSTRUCT("Invalid definition, STRUCT name already defined");   // *JiK*
runtime_error STRUCTwithoutENDS ("STRUCT without ENDS");            // *JiK*
runtime_error ENDSOutOfSTRUCT ("ENDS outside of STRUCT");           // *JiK*

runtime_error InvalidDGValue ("Failed to parse DG value string");   // *JiK*
runtime_error InvalidDGLength ("DG value is not multiple of 8");    // *JiK*
...
public:
    ...
    DebugType getdebugtype() const;     // *JiK*
    void errtostdout();
...
private:
    ...
    void parseBANK (Tokenizer & tz,                             // *JiK*
                const std::string & label= std::string () );
    void parseSTRUCT (Tokenizer & tz,                           // *JiK*
                const std::string & label= std::string () );
    void parseDG (Tokenizer & tz );                             // *JiK*
...
Asm::DebugType Asm::In::getdebugtype() const
{
    return debugtype;
}
...
const byte * Asm::In::getmem(int pos) const     // *JiK*
{
    return mem+pos;
}
            ...
            case TypeDEFM:
                for (;;) {
                    ...
                    count += l;
                            ...
                            if (tok.type () == TypeEndLine)
                                    break;
                            checktoken (TypeComma, tok, getline());  // *JiK*
                }
                break;
            case TypeDEFW:
            case TypeDW:
...
byte Asm::In::parsedesp(Tokenizer & tz, bool bracket)
{
    ...
    // *JiK* start - changing the IX/IY inder handling..
#if defined(OLD_PASMO_IXIY_INDICES)
    case TypePlus:
        tok = tz.gettoken();
        {
...
// *JiK* start
int Asm::getnumbanks() const
{
    return pin->getnumbanks();
}
void Asm::setcurrentbank(int bank)
{
    // Somewhat ugly hack to access private member
    pin->setcurrentbank(bank);
}

bool Asm::iscurrentbankused() const
{
    return pin->iscurrentbankused();
}


int Asm::In::getnumbanks() const
{
    // Somewhat ugly hack to access private member
    return mem.getnumbanks();
}
void Asm::In::setcurrentbank(int bank)
{
    // Somewhat ugly hack to access private member
    mem.setcurrentbank(bank);
}

bool Asm::In::iscurrentbankused() const
{
    return mem.iscurrentbankused();
}

address Asm::In::getcurrentbankminused() const
{
    return mem.getcurrentbankminused();
}

address Asm::In::getcurrentbankmaxused() const
{
    return mem.getcurrentbankmaxused();
}

address Asm::In::getcurrentbankused() const
{
    return mem.getcurrentbankused();
}

bool Asm::In::gotpaged() const
{
    return mem.gotpaged();
}

void Asm::emitpagedtap (std::ostream & out)
{
    pin->message_emit ("PAGED TAP");
    int bank;
    ...
    for (bank = 0; bank < pin->getnumbanks(); bank++) {
        pin->setcurrentbank(bank);

        if (pin->iscurrentbankused()) {
            std::ostringstream oss;
            oss << bank;
            
            minused = pin->getcurrentbankminused();
            maxused = pin->getcurrentbankmaxused();
            codesize = pin->getcurrentbankused();
            ...
            if (((minused ^ maxused) & ~(Bank::banksize-1)) != 0) {
                std::string warning = "Warning: BANK " + hex2str(bank) + " was accesses through both " \
                    "fixed and banked pages (minused: $" + hex4str(minused) + ", " \
                    "maxused: $" + hex4str(maxused) + "\n";
                pin->emitwarning(warning);
            }

            //
            headername = "bank" + oss.str() + ".tap";

            if (pin->getdebugtype() != NoDebug) {
                std::string msg = "Emiting BANK" + hex2str(bank) + " as '" + headername + \
                    "' from " + hex4str(minused) + " to " + hex4str(minused+codesize-1) + \
                    + "\n";    
                pin->showdebugmsg(msg);
            }
            // we know max bank size is 16K thus it will always fit into 
            // 16 bit address type
            for (pos = 0; pos < codesize; pos++) {
                bankmem[pos] = peekbyte(minused+pos);
            }
...
void Asm::emittapbas128(std::ostream & out)
{
    if (pin->gotpaged()) {
        if (pin->getdebugtype() != NoDebug)
            pin->showdebugmsg("Emiting TAP 128K basic loader");

        std::string basic (spectrum::pagedbasicloader (*this) );
        ...
        if (pin->getdebugtype() != NoDebug)
            pin->showdebugmsg("Switching to normal TAP basic loader");
...
void Asm::emittap128(std::ostream & out)
{
    if (pin->gotpaged()) {
        if (pin->getdebugtype() != NoDebug)
            pin->showdebugmsg("Emiting TAP 128K");

        // Prepare datas for banked output..
        emitpagedtap (out);
    } else {
        if (pin->getdebugtype() != NoDebug)
            pin->showdebugmsg("Switching to normal TAP");
